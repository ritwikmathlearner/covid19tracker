    ...
    if(!confirmed){
        ...
        const calculateData = ()=>{
            if(beforePastDayConfirmed && ((new Date(beforePastDay).toDateString() !== new Date(lastUpdate).toDateString()))){
                confirmedDifference = (parseInt(beforePastDayConfirmed) - parseInt(confirmed.value)) >= 0 ? `${(parseInt(beforePastDayConfirmed) - parseInt(confirmed.value))} decreased since ${new Date(beforePastDay).toDateString()}` : `${(parseInt(confirmed.value) - parseInt(beforePastDayConfirmed))} increased since ${new Date(beforePastDay).toDateString()}`;
                recoveredDifference = (parseInt(beforePastDayRecovered) - parseInt(recovered.value)) >= 0 ? `${(parseInt(beforePastDayRecovered) - parseInt(recovered.value))} decreased since ${new Date(beforePastDay).toDateString()}` : `${(parseInt(recovered.value) - parseInt(beforePastDayRecovered))} increased since ${new Date(beforePastDay).toDateString()}`;
                deathsDifference = (parseInt(beforePastDayDeaths) - parseInt(deaths.value)) >= 0 ? `${(parseInt(beforePastDayDeaths) - parseInt(deaths.value))} decreased since ${new Date(beforePastDay).toDateString()}` : `${(parseInt(deaths.value) - parseInt(beforePastDayDeaths))} increased since ${new Date(beforePastDay).toDateString()}`;
            }
        }
        calculateData();
    }
