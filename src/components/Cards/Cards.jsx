...
const Cards = ({ data: {confirmed, recovered, deaths, lastUpdate}, beforePastData: {beforePastDayConfirmed, beforePastDayRecovered, beforePastDayDeaths}, beforePastDay} ) => {
    ...
    if(!confirmed){
        ...
        const calculateData = ()=>{
            if(beforePastDayConfirmed && ((new Date(lastUpdate).toDateString()) !== (new Date(beforePastDay).toDateString()))){
    ...
    return (
        <div className={styles.container}>
            <Box p={5}>
                ...
                <Typography color="primary" variant="h5" className={styles.greeting} align="center">{confirmedDifference ? `Differences are based on ${new Date(beforePastDay).toDateString()}` : 'Welcome! It\'s your first day'}</Typography>
            </Box>
